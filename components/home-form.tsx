    ...
    <div>
      <form onSubmit={handleSubmit} className="space-y-6 sm:space-y-8">
        <div className="relative">
          <label htmlFor="repo-url" className="block mb-2 text-sm font-semibold text-neutral-200">
            GitHub Repository URL
          </label>
          <div className="relative group">
            <Github
              className={`absolute left-3 sm:left-4 top-1/2 transform -translate-y-1/2 h-4 w-4 sm:h-5 sm:w-5 transition-colors duration-200 ${
            ...
            <input
              ...
              className={`pl-10 sm:pl-12 pr-4 sm:pr-36 h-12 sm:h-14 w-full hover:shadow-2xl hover:shadow-neutral-900 rounded-xl border-2 transition-all duration-200 text-sm sm:text-base font-medium
                ${
                  isFocused
                    ? "bg-neutral-800 shadow-lg shadow-neutral-500/10"
            ...
            className={`sm:hidden mt-4 w-full h-12 text-sm font-semibold rounded-xl transition-all duration-300 flex items-center justify-center gap-2
              ${
                isLoading || !inputVal.trim()
                  ...
                  : "bg-neutral-200 text-neutral-900 hover:bg-neutral-300 shadow-md hover:shadow-lg"
              }`}
          >
            {isLoading ? (
              <>
        ...
        {error && (
          <div className="relative">
            <div className="flex items-start gap-3 text-red-400 text-sm bg-red-900/20 p-3 sm:p-4 rounded-xl border border-red-800/50">
        ...
        {isLoading && (
          <div className="space-y-3">
            <div className="flex flex-col sm:flex-row sm:justify-between gap-2 sm:gap-0 text-xs text-neutral-400">
              <span className="text-center sm:text-left">
                {isFetchingRepo && "Fetching the repository..."}
                {isReading && "Reading project files..."}
                {isGenerating && "Generating README with AI..."}
              </span>
              <span className="text-center sm:text-right text-neutral-500">This may take a few moments</span>
            </div>
